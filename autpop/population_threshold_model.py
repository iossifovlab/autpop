...
if __name__ == "__main__":
    # model = Model("aaa", 10, 11,
    #               [PopulationVariant(0.1, 1)] * 5 +
    #               [PopulationVariant(0.8, 2)] * 3)
    # model = Model("bbb", 1, 2,
    #               [PopulationVariant(0.4, 1)] * 1 +
    #               [PopulationVariant(0.7, -15)] * 1)

    models = ["Strong, rare risk", "Uniform low, rare risk",
              "Common risk with protection",
              "Common risk with protection, and females"]
    models = ["abba_toy2", "abba_toy2_A"]
    models = ["abba simple", "abba simple A"]
    # models = ["Common risk with protection, and females"]
    for model_name in models:
        print(f"Working on {model_name}")
        model = Model.load(model_name)
        all_stats, global_stats = model.compute_stats(
            family_mode="all", family_stats_mode="all", n_processes=None)
        # all_stats_O, global_stats_O = model.compute_stats(
        #     family_mode="sample", family_stats_mode="sample", n_processes=None)
        save_global_stats_buff([global_stats])

    # gsb = [global_stats_A]
    # for r in range(10):
    #     all_stats_B, global_stats_B = model.compute_stats(
    #         family_mode="sample", family_stats_mode="all", n_processes=None)
    #     gsb.append(global_stats_B)

    # save_global_stats_buff_summary(gsb)

    # ft = FamilyType(5, 6, [1], [], [5], [])
    # a_stats = ft.measure_stats_analytical()
    # s_stats = [ft.measure_stats_by_simulation(100000) for _ in range(1000)]

    # pl.clf()
    # for att_i, att in enumerate(['male_risk', 'female_risk',
    #                              'ma_ma_mom_SLC_mean', 'ma_ma_dad_SLC_mean',
    #                              'mu_ma_mom_SLC_mean', 'mu_ma_dad_SLC_mean']):
    #     pl.subplot(2, 3, att_i+1)
    #     pl.hist([ss[att] for ss in s_stats], bins=30)
    #     pl.plot([a_stats[att], a_stats[att]], pl.ylim())
    #     pl.title(att)
    # pl.tight_layout()
    # pl.show(block=False)
...
if __name__ == "___main__":
    ...
    number_of_family_types = 100000
    number_of_children_per_family = 100000
    number_of_runs = 1

    # models = ["debug", "unified hypothesis", "suppressor", "no suppressor"]
    models = ["Strong, rare risk", "Uniform low, rare risk",
              "Common risk with protection",
              "Common risk with protection, and females"]
    models = ["abba_original", "abba_toy2"]
    for model_name in models:
        model = Model.load(model_name)
        global_stats_buff = []

        res_dir = pathlib.Path(f'./results/{model.model_name}')
        res_dir.mkdir(parents=True, exist_ok=True)

        for rn in range(number_of_runs):
            print(f"\n\nModel {model_name}, run {rn}...")

            model.sample_family_types(number_of_family_types)
            all_stats = model.generate_family_type_statistics(
                number_of_children_per_family)
            add_acertainment_stats(all_stats, number_of_family_types)
            global_stats_buff.append(
                compute_global_stats(all_stats, model))
            if rn > 0:
                continue

            # draw_dashboard(model, all_stats,
            #                res_dir / "dashboard.png", show=True)
            save_stats(all_stats, res_dir / "family_types.txt")

        save_global_stats(model, global_stats_buff,
                          res_dir / f"global_report_{model_name}.txt")
        save_global_stats(model, global_stats_buff)
